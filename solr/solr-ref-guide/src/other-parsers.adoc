...
The `score` doesn't allow you to specify a `method` parameter
====

=== Join Parser Execution

The join parser can choose between two methods of execution. The "method" parameter is described below

*method*

`enum`::
Default approach where we enumerate over the indexed terms and check each term for an intersection. The execution time depends on O(field_cardinality)
`dv`::
For every document in the domain, we look up it's values. The execution time depends on O(number_of_matched_docs) . The "from" field must have doc-values enabled for this method. This is the only execution method for point fields.
